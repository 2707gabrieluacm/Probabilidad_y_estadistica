...
1.  calcular la expresion cos(pi/6+pi/2)+e\^2 2 calcular la expresion
    cos(pi/6+pi/2)+e\^2\*log(5)+arc cos(1/raiz(2)) 3 introducir las
...
instruccion 1 instruccion 2 return(valor_de_retorno) }
...
Generemos los datos correspondientes a los pesos registrados de 90
personas
...
Con la misma idea que la gráfica anterior, generemos un histograma de
los mismos datos
...
Mejoremos el gráfico agregando detalles, leyendas e información en la
parte lateral de la gráfica
...
## Ejemplo 6. Considera los siguientes tipos de variables Nominales:

Volvemos a limpiar nuestro ambiente, eliminamos variables y cerramos las
gráficas generadas con anterioridad.
...
```{r}
frecuencias <- table(sexo); (frecuencias)
```

Calculamos la frecuencia en por centaje
```{r}
proporciones <- prop.table(frecuencias) * 100; (proporciones)
```
...
barplot(frecuencias,
        ...
        xlab = "Sexo",
        ylab = "Frecuencia",
        col = c("lightblue", "salmon", "lightgray"))
```

Creamos una grafica de pastel
```{r}
etiquetas_pastel <- paste0(names(proporciones), " (", round(proporciones, 1), "%)")

pie(frecuencias,
    main = "Distribución de Sexo por Porcentaje",
